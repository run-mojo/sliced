...
use std::mem::{self, align_of_val, size_of_val};
use std::ops::Deref;
...
//            data,
//        };
        Arc { ptr: boxed_non_null(ArcInner {
            strong: atomic::AtomicUsize::new(1),
            weak: atomic::AtomicUsize::new(1),
            data,
        }), phantom: PhantomData }
...
impl<T: ?Sized> Arc<T> {
    ...
    unsafe fn drop_slow(&mut self) {
        ...
        if self.inner().weak.fetch_sub(1, Release) == 1 {
            atomic::fence(Acquire);
            dealloc(self.ptr.as_ptr() as *mut _ as *mut u8);
...
//        let inner = set_data_ptr(ptr as *mut T, mem.as_ptr() as *mut u8) as *mut ArcInner<T>;
    ...
    fn from_box(v: Box<T>) -> Arc<T> {
        unsafe {
            ...
            dealloc(box_unique.as_ptr() as *mut _ as *mut u8);
//            box_free(box_unique);
        ...
        impl<T> Drop for Guard<T> {
            fn drop(&mut self) {
                ...
                unsafe {
                    ...
                    dealloc(self.mem.as_ptr() as *mut _ as *mut u8);
